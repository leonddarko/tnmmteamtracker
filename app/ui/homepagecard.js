    ...
    return (<>
        <div className="">
            <div className="px-6 pt-12 pb-6 rounded-t-2xl bg-cyan-50 shadow-sm">
                ...
                <h1 className=" text-5xl font-bold text-black max-w-md mb-2">Manual Entry & Progress Tracker</h1>

                {status === "loading" && (<span className="loading loading-spinner loading-xs text-cyan-950"></span>)}

                {status === "authenticated" && (
                    <div className=" text-sm text-zinc-700 mb-2">Welcome, {session?.user.name}.</div>
                )}
                {status === "authenticated" && (
                    <>
                        <Image
